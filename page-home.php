...
<div class="hero-slider">
    ...
    <?php while (have_rows('carousel_images')) : $the_row = the_row();
        ...
        <?php foreach ($the_row as $field_key => $field_value) {
            ...
            $preColour = 'colour_';
            $colourName = $preColour . $i;
            $colour = get_sub_field($colourName);
        ?>
            <?php if ($image) : ?>
                <div class="hero-slider__slide <?php if ($i !== 1) { ?>fade<?php } ?>" <?php if ($colour) : ?>data-theme="<?php echo $colour; ?>" <?php endif; ?> style="background-image: url('<?php echo esc_url($image['url']); ?>');">
                </div>
            <?php endif; ?>
            <?php $i++; ?>
        <?php } ?>
...
if ($lacarte) : ?>
    <div class="homepage--menu-section">
        <div class="half--block image" style="background-image: url('<?php echo $lacarte['image']; ?>')"></div>
        <div class="half--block text">
            ...
            <div>
                ...
                <?php if (empty($lacarte['link_text'])) { ?>
                    <div class="logo-small" data-open-popup class="open-menu-button" style="background-image: url('<?php echo get_theme_file_uri('/assets/images/sugaar_logo.png') ?>')"></div>
                <?php
                } else {
                ?>
        ...
        <div class="half--block text">
            ...
            <div>
                <div data-open-popup class="open-menu-button">
                    <?php if (empty($lacarte['link_text'])) { ?>
                        <div class="logo-small" data-open-popup class="open-menu-button" style="background-image: url('<?php echo get_theme_file_uri('/assets/images/dragon_logo.svg') ?>')"></div>
                    <?php  } else {
                        echo $lacarte['link_text'];
                    }
                    ?>
...
<div class="reservations--section" data-close-reservations>
    ...
    <div class="iframe--container">
        <iframe allowtransparency="true" name="reservations-iframe" id="reservations-iframe" src="https://bookings.zenchef.com/results?rid=358531&pid=1001" frameborder="0">
        </iframe>
    </div>
</div>
